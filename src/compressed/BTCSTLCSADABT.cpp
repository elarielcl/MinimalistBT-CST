#include <compressed/BTCSTLCSADABT.h>
#include <sdsl/suffix_trees.hpp>

BTCSTLCSADABT::BTCSTLCSADABT(std::string& input_string, int block_tree_version, int r, int leaf_length,
                             int block_size) {
    ...
    // DABT DISA CONSTRUCTION, FIXED LOW MEMORY CONFIGURATION
    ...
    SumBlockTree* sbt = new SumBlockTree(disa, 2, 4);
    sbt->process_back_pointers_heuristic();
    sbt->add_access_support();
...
BTCSTLCSADABT::BTCSTLCSADABT(std::ifstream& in) {
...
int BTCSTLCSADABT::first_child(int node) {
    return btct_->first_child(node);
}


int BTCSTLCSADABT::tree_depth(int node) {
    return btct_->tree_depth(node);
}


int BTCSTLCSADABT::next_sibling(int node) {
    return btct_->next_sibling(node);
}


int BTCSTLCSADABT::parent(int node) {
    return btct_->parent(node);
}


int BTCSTLCSADABT::level_ancestor(int node, int d) {
    return btct_->level_ancestor(node, d);
}


int BTCSTLCSADABT::lca(int node1, int node2) {
    return btct_->lca(node1, node2);
}


int BTCSTLCSADABT::suffix_link(int node) {
...
int BTCSTLCSADABT::string_depth(int node) {
    if (node == 0) return 0;
...
int BTCSTLCSADABT::string_ancestor(int node, int d) {
...
int BTCSTLCSADABT::child(int node, int c) {
    if (c == 0) c = '$';
...
int BTCSTLCSADABT::bin_search_child(int node, int c) {
    if (c == 0) c = '$';
    int sdepth = string_depth(node);
    int child = node+1;
    int j = -1;
...
int BTCSTLCSADABT::string(int node, int i) {
    if (node == node_ && i == i_+1) {
        ...
        if (i > disa_constant) psi_ = disa_->access((*dsa_)[lr]+i);
...
int BTCSTLCSADABT::size() {
    return btct_->size() + rlcsa_->getSize() + lcp_rlcsa_->getSize() + dsa_->size_in_bytes() + disa_->size();
}


void BTCSTLCSADABT::serialize(std::ofstream& out) {
